...
def show_box(box, ax, label):
    ...
    ax.text(x0+w/2, y0+h/2, label, color='white', fontsize=8)
...
def main():
    ...
    """
    **NOTE:** To get better Grounding DINO detection we will leverage a bit of prompt engineering using `enhance_class_name` function defined below. 
    You can learn more from our [Grounding DINO tutorial](https://blog.roboflow.com/grounding-dino-zero-shot-object-detection/).
    """
    tags = enhance_class_name(tags)
    ...
    boxes_filt, scores, pred_phrases = get_grounding_output(
        grounding_dino_model, i_tensor, tags, box_threshold, text_threshold, device=device
    )
    print(pred_phrases)
    ...
    # drop all bbox without text
    score_idx = [i for i, score in enumerate(scores[nms_idx]) if score > text_threshold]
    boxes_filt = boxes_filt[score_idx]
    pred_phrases = [pred_phrases[idx] for idx in score_idx]
    ...
    def segment(sam_predictor: SamPredictor, xyxy: np.ndarray) -> np.ndarray:
        result_masks = []
        for box in xyxy:
    ...
    ax0.axis('off')
    # for mask in masks:
    #     show_mask(mask, ax0, random_color=True)
    for box, label in zip(boxes_filt, pred_phrases):
        show_box(box, ax0, label)
